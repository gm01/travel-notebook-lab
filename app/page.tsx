  ...
  return <main id="top">
    ...
    <div className="container">
      ...
      <section className="section" id="stays">
        <Heading no="03" title="확정 숙박 계획" note="4곳 · 13박 · 성인 2명" />
        <div className="availability"><span>숙박비 합계</span><p><strong>{formatKRW(lodgingTotalKRW)}</strong> · 13박 · 원화 기준</p></div>
        ...
        <p className="footnote">우붓은 거리 주소만 있어 정확한 숙소 핀과 시설을 확인해야 합니다. 추가 비용·조식·취소 조건은 예약 원본을 확인하세요.</p>
      </section>
      <section className="section" id="spots">
        <Heading no="04" title="먹고, 보고, 쉬기" note="식당·카페·생활 편의. 우붓 후보는 숙소의 정확한 위치와 거리를 확인하세요." />
      ...
      <section className="section budget-section" id="budget">
        <Heading no="06" title="두 사람의 예산" note="숙박 외 항목은 예상 범위입니다. 항공권·쇼핑·여행자보험 제외." />
        <div className="budget-grid"><div><p className="eyebrow">13 NIGHTS · 2 PEOPLE</p><h3>13박 숙박비,<br />{formatKRW(lodgingTotalKRW)}</h3><p>숙박비는 원화, 현지 예산은 루피아 기준입니다. 총예산은 결제 시점 환율로 계산하세요.</p><Link href={notionUrl}>노션 예산표에 실제 결제액 기록</Link></div>
      ...
      <section className="source-section" aria-label="정보 출처">
        <div><p className="eyebrow">NOTEBOOK & SOURCES</p><h2>예약 메모와 참고 링크</h2><p>운영시간·요금은 방문 전 공식 안내를 확인하세요. 노션을 수정해도 사이트가 자동으로 바뀌지는 않습니다.</p><Link href={notionUrl} className="notion-button">여행 노션으로</Link></div>
